...
- **Unauthenticated 401s remain absent**, deliberately. There is no principal to attribute them to, and 401 is the one refusal an anonymous caller can trigger at will, so recording it would let unauthenticated traffic drive unbounded database writes. That count lives in `/metrics`, so a caller-visible error total needs both sources — see `docs/operations.md`.
