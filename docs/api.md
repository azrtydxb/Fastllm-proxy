...
- **Requests that never reached a backend are still absent**, and deliberately so: the row is written by the body that forwards the response, and a request refused by the proxy has no such body. 401/403, 429, 402 and the synthesised 502 for an unreachable chain therefore leave no `usage_events` row — confirmed against a model pointed at a dead address. `usage_events` answers "what did callers consume, and what did upstreams do with it"; `/metrics` answers "what did callers see". See `docs/operations.md` for why a chart must not present the first as the second.
