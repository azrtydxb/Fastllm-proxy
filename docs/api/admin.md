...
| `GET /admin/providers`                       | Every provider: its kind (`static`, `cloud`, `dynamic`), endpoint, protocol, whether a credential is set, and how many models ride on it. A provider used to be a grouping the UI invented at render time; it is a row |
