...
mod tests {
    ...
    async fn migrations_apply_and_seed_the_default_roles() {
        ...
        // `mcp:invoke` and `agent:invoke` are here and on neither `inference`
        // nor `operator`: a key that may invoke models is not, by that fact, a
        // key that may reach every tool server or set every agent running.
        // Tools and agents have side effects; models do not. This assertion is
        // the thing that would notice a migration quietly widening either of
        // those roles.
        assert_eq!(
            grants("admin"),
            vec![
